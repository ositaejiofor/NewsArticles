...
SLEEP_INTERVAL=10  # seconds between status checks
...
# ==============================
# 6. Verify site and HTTPS
# ==============================
RENDER_URL="https://$RENDER_SERVICE_NAME.onrender.com"
CUSTOM_URL="https://$CUSTOM_DOMAIN"

echo "=== Checking Render default domain ==="
HTTP_STATUS=$(curl -o /dev/null -s -w "%{http_code}\n" $RENDER_URL)
if [ "$HTTP_STATUS" -eq 200 ]; then
    echo "✅ Render default domain is live: $RENDER_URL"
else
    echo "⚠️ Render default domain may not be live yet. Status code: $HTTP_STATUS"
fi

echo "=== Checking custom domain ==="
HTTP_STATUS_CUSTOM=$(curl -o /dev/null -s -w "%{http_code}\n" $CUSTOM_URL)
if [ "$HTTP_STATUS_CUSTOM" -eq 200 ]; then
    echo "✅ Custom domain is live with HTTPS: $CUSTOM_URL"
else
    echo "⚠️ Custom domain not live yet. Make sure Cloudflare DNS is set correctly."
fi
...
# ==============================
echo "===================================================="
echo "Cloudflare DNS settings (copy to Cloudflare if not done):"
echo "Type: CNAME"
echo "Name: www"
